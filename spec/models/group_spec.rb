...
RSpec.describe Group, type: :model do
  before(:each) do
    ...
    @test_e = Saving.create(name: 'test saving', author_id: @test_u.id, amount: 50.0)
    @test_g = Group.create(name: 'test group', creator_id: @test_u.id)
    Groupedsaving.create(group_id: @test_g.id, saving_id: @test_e.id)
  end
  ...
  it '4. Checks for the group and saving models association' do
    expect(@test_g.savings).not_to be_empty
  end
